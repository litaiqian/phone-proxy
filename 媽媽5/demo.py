...
def _post(url, headers=None, json=None, data=None, proxy=None, **kwargs):
    ...
    if proxy:
        try:
            ...
            err_str = str(e)
            # TLS 错误(35) / 连接超时(28) / 连接拒绝(7) / 代理错误(5,97)
            if any(x in err_str for x in ('curl: (35)', 'curl: (28)', 'curl: (7)', 'curl: (5)', 'curl: (97)',
                                          'TLS connect error', 'Connection timed out',
                                          'Connection refused', 'Failed to connect')):
                print(f'[{datetime.now().strftime("%H:%M:%S.")}{datetime.now().microsecond // 1000:03d}] '
                      f'[代理断开] {proxy[:50]} ({err_str[:80]})')
                print(f'           → 回退直连')
...
def _get(url, headers=None, params=None, proxy=None, **kwargs):
    ...
    代理 TLS/连接失败时自动回退直连。"""
    kwargs.pop('impersonate', None)
    _timeout = kwargs.pop('timeout', 15)
    if proxy:
        try:
