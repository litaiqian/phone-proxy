...
# 服务端地址列表（按优先级排序）
SERVER_ADDRESSES = [
    "http://127.0.0.1:5000",           # 本地回环地址（本机运行）
    "http://localhost:5000",            # localhost（本机运行）
]
...
local_ip_url = get_local_ip()
if local_ip_url:
    SERVER_ADDRESSES.append(local_ip_url)

# 可选：手动添加外网地址（如果有）
# 如果服务端在外网，取消下面的注释并填入实际外网IP
# SERVER_ADDRESSES.append("http://你的外网IP:5000")
...
def detect_working_server() -> Optional[str]:
    """
    检测可用的服务端地址
    返回: 可用的服务器地址，None表示都不可用
    """
    print(f"[服务端] 正在检测可用的服务端地址...")
    print(f"[服务端] 待检测地址列表: {', '.join(SERVER_ADDRESSES)}")
    
    for addr in SERVER_ADDRESSES:
        try:
            resp = requests.get(
                f"{addr}/api/client/get_config",
                headers={'X-API-TOKEN': API_TOKEN},
                timeout=SERVER_TIMEOUT
            )
            if resp.status_code == 200:
                print(f"[服务端] ✅ 发现可用服务端: {addr}")
                return addr
            else:
                print(f"[服务端] ⚠️ {addr} 响应异常: HTTP {resp.status_code}")
        except requests.exceptions.ConnectionError:
            print(f"[服务端] ❌ {addr} 连接失败")
        except requests.exceptions.Timeout:
            print(f"[服务端] ⏱️ {addr} 连接超时")
        except Exception as e:
            print(f"[服务端] ⚠️ {addr} 异常: {e}")
    
    return None


def wait_for_server() -> Optional[str]:
    """
    等待服务端上线，随机间隔5-20秒重试，直到连接成功
    返回: 可用的服务器地址，None表示用户中断
    """
    retry_count = 0

    while True:
        working_server = detect_working_server()
        if working_server:
...
def fetch_rush_time(server_url: str) -> Optional[Tuple[int, int]]:
    """
    从服务端获取抢购时间配置
    返回: (hour, minute) 元组，失败返回 None
    """
    url = f"{server_url}/api/client/get_config"
    headers = {'X-API-TOKEN': API_TOKEN}
...
def report_result(server_url: str, phone: str, success: bool, order_id: str = "", h5_url: str = "", error: str = ""):
    """上报抢购结果到服务端"""
    url = f"{server_url}/api/client/report_result"
...
def rush_and_pay_for_account(server_url: str, task: dict, item_code: str, act_id: str):
    ...
    for attempt in range(1, MAX_RUSH_ATTEMPTS + 1):
        ...
        # 抢购超时失败
        report_result(server_url, phone, success=False, error="抢购超过最大尝试次数")
        return
    ...
    try:
        ...
        if not addresses:
            report_result(server_url, phone, success=False, error="无收货地址")
            return
        default_addr = next((addr for addr in addresses if addr.get("dft")), addresses[0])
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"获取地址失败: {e}")
        return

    record_id = rush_result.get("data", {}).get("priorityRecordId", 0)
    if not record_id:
        report_result(server_url, phone, success=False, error="未获取到 priorityRecordId")
        return
    ...
    try:
        ...
        if compose_res.get("code") != 2000:
            report_result(server_url, phone, success=False, error=f"组单失败: {compose_res}")
            return
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"组单异常: {e}")
        return
    ...
    try:
        ...
        if submit_res.get("code") != 2000:
            report_result(server_url, phone, success=False, error=f"下单失败: {submit_res}")
            return
        order_id = submit_res.get("data", {}).get("orderId")
        if not order_id:
            report_result(server_url, phone, success=False, error="未获取到 orderId")
            return
        print(f"[{phone}] 下单成功，订单号: {order_id}")
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"下单异常: {e}")
        return
    ...
    try:
        ...
        if pay_res.get("code") != 2000:
            report_result(server_url, phone, success=False, error=f"支付请求失败: {pay_res}")
            return
        channel_trade_sn = pay_res.get("data", {}).get("channelTradeSn")
        if not channel_trade_sn:
            report_result(server_url, phone, success=False, error="未获取到 TN")
            return
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"支付异常: {e}")
        return
    ...
    try:
        ...
        if code not in (200, 2000):
            report_result(server_url, phone, success=False, error=f"支付网关失败: {gw_result}")
            return
        ...
        if not sdk_str:
            report_result(server_url, phone, success=False, error="支付网关未返回 SDK 串")
            return
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"支付网关异常: {e}")
        return
    ...
    try:
        ...
        if not h5_result.get("success"):
            report_result(server_url, phone, success=False, error=f"转链失败: {h5_result.get('message')}")
            return
        h5_url = h5_result["h5Url"]
        print(f"[{phone}] 获取到支付链接: {h5_url[:80]}...")
    except Exception as e:
        report_result(server_url, phone, success=False, error=f"转链异常: {e}")
        return

    # 8. 上报最终成功结果
    report_result(server_url, phone, success=True, order_id=order_id, h5_url=h5_url)


def fetch_tasks(server_url: str) -> List[dict]:
    """从服务端拉取任务列表"""
    url = f"{server_url}/api/client/get_tasks"
    headers = {'X-API-TOKEN': API_TOKEN, 'Content-Type': 'application/json'}
...
def fetch_config(server_url: str) -> Tuple[str, str, Optional[Tuple[int, int]]]:
    """
    获取抢购商品配置和抢购时间
...
def execute_rush_cycle(server_url: str):
    """
    执行一次完整的抢购周期
    ...
    # 1. 获取商品配置和抢购时间
    item_code, act_id, rush_time = fetch_config(server_url)
    ...
    threads = []
    for task in tasks:
        t = threading.Thread(target=rush_and_pay_for_account, args=(server_url, task, item_code, act_id))
        t.daemon = True
        t.start()
        threads.append(t)
        # 避免同时启动太多，错开一点
...
def main():
    print("=" * 60)
    print("=== i猫妈妈分布式抢购客户端启动 (FastAPI版) ===")
    print("=" * 60)
    print(f"[配置] 已配置 {len(SERVER_ADDRESSES)} 个服务端地址:")
    for i, addr in enumerate(SERVER_ADDRESSES, 1):
        addr_type = "本地" if "127.0.0.1" in addr or "localhost" in addr else "内网"
        print(f"  {i}. {addr} ({addr_type})")
    print(f"\n[提示] 客户端将自动检测可用的服务端地址")
    print(f"       如需添加外网地址，请编辑代码中的 SERVER_ADDRESSES\n")
    ...
    while True:
        try:
            ...
            print(f"\n[客户端] ✅ 已连接到服务端: {server_url}\n")

            # 2. 执行抢购周期
            execute_rush_cycle(server_url)
            ...
            if tomorrow_rush > 0:
                ...
                while elapsed < tomorrow_rush:
                    ...
                    print(f"[等待] 下次检查: {check_interval}秒 | 距离抢购: {hours:02d}:{minutes:02d}:{seconds:02d}",
                          end='\r')
                    ...
                    try:
                        resp = requests.get(
                            f"{server_url}/api/client/get_config",
                            headers={'X-API-TOKEN': API_TOKEN},
                            timeout=2
                        )
                        if resp.status_code == 200:
