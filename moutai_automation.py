...
# 不直接导入 demo.py，MoutaiClient 操作通过 HTTP 桥接调用 main.py（端口 8000）
BRIDGE_BASE_URL = 'http://127.0.0.1:8000'


class BridgeClient:
    """通过 HTTP 桥接调用 main.py 的 MoutaiClient 操作，避免直接导入 demo.py"""
    ...
    async def _post(self, endpoint: str, data: dict) -> dict:
        ...
        try:
            ...
            print(f'[桥接] 无法连接到 main.py ({self.base_url})，请确保 main.py 正在运行')
            return {'success': False, 'message': '桥接服务不可用'}
        except Exception as e:
            print(f'[桥接] 请求异常: {e}')
            return {'success': False, 'message': str(e)}
    
    async def check_login(self, credentials: dict, proxy_url: str = '') -> bool:
        """检查登录状态"""
...
# ===================== 手机代理池（内存热缓存） =====================
phone_proxy_pool: dict = {}       # {proxy_id: {"proxy_addr": "socks5://...", "name": "...", "tailscale_ip": "...", "last_heartbeat": float}}
PHONE_PROXY_TIMEOUT = 90          # 超时（秒），超过此时间未心跳视为离线
phone_proxy_pool_lock = asyncio.Lock()
...
class PhoneProxy(Base):
    ...
    tailscale_ip = Column(String(45), default='')                # Tailscale IP
    port = Column(Integer, default=10808)
...
async def check_login_validity_async(phone: str, proxy_url: str = '') -> bool:
    """通过桥接异步检查登录状态"""
...
async def client_get_pause_status(request: Request, db: SQLSession = Depends(get_db)):
    ...
    proxy_enabled = up.proxy_enabled
    proxy_url = up.proxy_url
    return JSONResponse(content={'paused': paused, 'proxy_enabled': proxy_enabled, 'proxy_url': proxy_url})
...
def _build_task_response(records: list, db=None, uploader_id=0) -> list:
    ...
    def _get_best_ip():
        """获取最优IP：手机代理(最优先) > 就绪池 > 原始池"""
        # 1. 优先从手机代理池取（手机蜂窝IP，CDN信任度最高）
        phone_ip = pick_phone_proxy_ip()
        if phone_ip:
            return phone_ip
        # 2. 其次从就绪池取
    ...
    tasks = []
    ip_usage_count = {}  # 本轮分配中每个IP的使用计数
    _assigned_new = False  # 本轮是否有新分配（非纯查询），有才打印汇总

    for r in records:
        proxy_ip = r.proxy_ip or ''

        # 自动分配代理：代理开启且该记录无代理IP时，从池中获取
        if proxy_enabled and not proxy_ip and proxy_api_url and db:
            ...
            if proxy_ip:
                ...
                try:
                    ...
                    pass
        elif proxy_enabled and proxy_ip and proxy_api_url:
    ...
    if _assigned_new and proxy_enabled and ip_usage_count:
...
@app.post("/api/phone_proxy/register")
async def phone_proxy_register(request: Request):
    """手机端注册代理：手机运行 phone_proxy.py 后调此端点注册"""
    if request.headers.get('X-API-TOKEN') != Config.API_TOKEN:
        raise HTTPException(status_code=403)
    data = await request.json()
    proxy_addr = data.get('proxy_addr', '')       # socks5://tailscale_ip:10808
    name = data.get('name', '')                    # 手机名称
    tailscale_ip = data.get('tailscale_ip', '')
    port = data.get('port', 10808)

    if not proxy_addr:
        return JSONResponse(content={'status': 'error', 'message': '缺少 proxy_addr'})

    try:
        db = SessionLocal()
        # 按 tailscale_ip+port 去重（同一手机重复注册更新记录）
        existing = db.query(PhoneProxy).filter(
            PhoneProxy.tailscale_ip == tailscale_ip,
            PhoneProxy.port == port
        ).first()
        if existing:
            existing.proxy_addr = proxy_addr
            ...
            pp = PhoneProxy(
                proxy_addr=proxy_addr, name=name,
                tailscale_ip=tailscale_ip, port=port,
                status='online', last_heartbeat=datetime.datetime.utcnow()
            )
            db.add(pp)
            db.commit()
            db.refresh(pp)
        ...
        # 更新内存池
        now = time.time()
        async with phone_proxy_pool_lock:
            phone_proxy_pool[proxy_id] = {
        ...
        print(f'[手机代理] 注册成功 ID={proxy_id} | {name} | {proxy_addr}')
        return JSONResponse(content={'status': 'success', 'proxy_id': proxy_id})
    except Exception as e:
        print(f'[手机代理] 注册异常: {e}')
        return JSONResponse(content={'status': 'error', 'message': str(e)[:100]})


@app.post("/api/phone_proxy/heartbeat")
async def phone_proxy_heartbeat(request: Request):
    """手机代理心跳：每30秒调用一次保持在线"""
    if request.headers.get('X-API-TOKEN') != Config.API_TOKEN:
        raise HTTPException(status_code=403)
    data = await request.json()
    ...
    if not proxy_id:
        return JSONResponse(content={'status': 'error', 'message': '缺少 proxy_id'})

    now = time.time()
    async with phone_proxy_pool_lock:
        if proxy_id in phone_proxy_pool:
            phone_proxy_pool[proxy_id]['last_heartbeat'] = now
        else:
            # 内存池不存在但数据库可能有，尝试从数据库恢复
            try:
                db = SessionLocal()
                pp = db.query(PhoneProxy).filter(PhoneProxy.id == proxy_id).first()
                db.close()
                if pp:
                    phone_proxy_pool[proxy_id] = {
                        'proxy_addr': pp.proxy_addr,
                        'name': pp.name or '',
                        'tailscale_ip': pp.tailscale_ip or '',
                        'last_heartbeat': now
                    }
            except Exception:
                pass
    ...
    try:
        ...
        pp = db.query(PhoneProxy).filter(PhoneProxy.id == proxy_id).first()
        if pp:
            pp.last_heartbeat = datetime.datetime.utcnow()
            pp.status = 'online'
            db.commit()
        db.close()
    except Exception:
...
def get_online_phone_proxies() -> list:
    """获取在线的手机代理列表（从内存池，按心跳时间过滤）"""
    now = time.time()
    online = []
    for pid, info in phone_proxy_pool.items():
        if now - info.get('last_heartbeat', 0) <= PHONE_PROXY_TIMEOUT:
            online.append(info)
    return online


def pick_phone_proxy_ip() -> str:
    """从在线手机代理池中轮询选取一个代理IP
    返回 socks5://... 或空字符串
    优先 WebSocket 隧道（无需 Tailscale），其次 Tailscale 直连"""
    # 方式1: WS 隧道（无需 Tailscale）
    tunnel = tunnel_manager.pick_best_tunnel()
    if tunnel:
        return f'socks5://{_get_server_ip()}:{tunnel.local_port}'
    # 方式2: Tailscale 直连
    online = get_online_phone_proxies()
    if not online:
        return ''
    idx = int(time.time() * 1000) % len(online)
    return online[idx].get('proxy_addr', '')
...
async def phone_proxy_websocket(ws):
    """手机 WebSocket 隧道端端
    手机 APK 连接此端点建立隧道，无需 Tailscale
    协议:
      手机 → 服务器: {"type": "register", "name": "phone_001"}
      服务器 → 手机: {"type": "registered", "tunnel_id": "...", "local_port": 10810}
      服务器 → 手机: {"type": "connect", "tunnel_id": "abc123", "host": "xxx.com", "port": 443}
      手机 → 服务器: {"type": "connected", "tunnel_id": "abc123"}
      手机 ↔ 服务器: 二进制 T{tunnel_id}{data}
    """
    await ws.accept()
    tunnel = None
    tunnel_id = ''
    ...
    try:
        ...
        # 创建隧道
        tunnel = tunnel_manager.add_tunnel(ws, name)
        ...
        await ws.send_json({
            ...
            'proxy_addr': f'socks5://{_get_server_ip()}:{tunnel.local_port}',
        })

        print(f'[手机WS] 隧道注册成功 tunnel_id={tunnel_id[:8]} name={name} port={tunnel.local_port}')
        ...
        while True:
            ...
            if 'bytes' in raw:
                tunnel.on_ws_data(raw['bytes'])
            elif 'text' in raw:
                try:
                    ctrl = json.loads(raw['text'])
                    if ctrl.get('type') == 'ping':
                        await ws.send_json({'type': 'pong'})
                    elif ctrl.get('type') == 'connected':
                        tid = ctrl.get('tunnel_id', '')
                        if tid in tunnel._pending:
                            tunnel._pending[tid].set()
                    elif ctrl.get('type') == 'error':
                        tid = ctrl.get('tunnel_id', '')
                        if tid in tunnel._pending:
                            tunnel._pending[tid].set()
                except Exception:
        ...
        print(f'[手机WS] 连接异常 tunnel={tunnel_id[:8] if tunnel_id else "?"}: {e}')
    finally:
        if tunnel_id:
...
def _get_server_ip() -> str:
    """获取服务器对外 IP"""
    # 直接返回服务器地址，客户端通过 socks5://ipla.top:PORT 连接隧道
    return 'ipla.top'
