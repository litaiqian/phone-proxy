...
class MoutaiClient:
    ...
    # ==================== 查询 ====================
    def query_order_list(self, page_num=1, page_size=20):
        """查询订单列表，获取中奖/待支付订单"""
        headers = self._app_headers(need_sign=False)
        resp = _get(f"{BASE_URL}/xhr/front/trade/order/list",
                    headers=headers,
    ...
    def get_winning_bid_result(self):
        ...
        orders = self.query_order_list()
        # 筛选状态：已中签/待支付/已完成等（根据实际返回字段）
        winning_orders = [o for o in orders if o.get("status") in (1, 2, 3)]  # 示例状态
        if winning_orders:
            # 取第一个中奖商品的名称
            return winning_orders[0].get("itemName", "中奖")
        return ""
    # ==================== 抢购 ====================
