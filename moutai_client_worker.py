...
if BAKED_USER_ID > 0:
    ...
    BRIDGE_BASE_URL = "http://ipla.top:8000"
    ...
    def parse_args():
        ...
        p.add_argument('--bridge', type=str, default='http://ipla.top:8000')
...
def get_pause_status():
    ...
    if r.get('paused') is not None:
        _proxy_cache = {'enabled': r.get('proxy_enabled', False), 'url': r.get('proxy_url', '')}
        return r
...
def rush_single_account(task, client, rush_count, stop_flag, same_ip_count=1, thread_index=0,
    ...
    # 全局代理关闭时，清除残留的代理信息，切换为直连模式
    if not _proxy_cache.get('enabled') and proxy_ip:
        task['proxy_ip'] = ''  # 清空任务中的代理IP
        client.proxy = None    # 客户端切直连
        with _client_pool_lock:
            if phone in _client_pool:
                _client_pool[phone].proxy = None  # 同步清空实例池中的代理
        proxy_ip = ''
...
async def async_main():
    ...
    # 全局代理关闭时，清除所有任务的代理IP（服务器分配时可能带上了代理）
    if not _proxy_cache.get('enabled'):
        for t in tasks:
            t['proxy_ip'] = ''
        log('[代理] 已清除所有任务的代理配置，全部直连')
    ...
    while True:
        ...
        if cycle_num > 1:
            ...
            try:
                ...
                if new_tasks:
                    # 代理关闭时清除服务端带回的proxy_ip（服务端分配时可能带上了代理）
                    if not _proxy_cache.get('enabled'):
                        for t in new_tasks:
                            t['proxy_ip'] = ''
        ...
        if _rush_mode == 0 or in_rush_window() or seconds_to_rush_window() <= RUSH_PRESTART:
            ...
            for _, t in active:
                ...
                if lm == 'direct':
                    ...
                    lock_parts.append(f'{t["phone"]}=代理({lm})🔒')
                else:
                    lock_parts.append(f'{t["phone"]}={"代理" if _use_proxy_this_round and t.get("proxy_ip") else "直连"}')
